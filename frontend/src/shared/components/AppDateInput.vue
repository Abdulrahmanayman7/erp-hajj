...
const displayLabel = computed(() => formatIsoDateAr(value.value, props.placeholder))
...
function syncViewFromValue(): void {
  if (!hasValue.value) {
    const now = new Date()
...
function eventInsidePicker(event: Event): boolean {
  ...
  return path.some((node) => {
    ...
    return Boolean(node.closest('[data-app-date-menu]'))
  })
}
...
<template>
  <div ref="root" class="relative w-full min-w-[11rem]">
    ...
    <button
      ...
      @click="toggle"
    >
      <CalendarDays class="h-4 w-4 shrink-0 text-brand-primary" :stroke-width="1.75" aria-hidden="true" />
      <span class="min-w-0 flex-1 truncate" :class="hasValue ? 'text-brand-text' : 'text-brand-text-muted'">
        {{ displayLabel }}
      </span>
      <span
