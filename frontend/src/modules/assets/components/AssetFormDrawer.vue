...
<template>
  <Teleport to="body">
    <div v-if="open" class="fixed inset-0 z-50" role="presentation">
      ...
      <aside
        class="absolute inset-y-0 start-0 flex w-full max-w-[560px] flex-col bg-brand-surface shadow-xl"
        role="dialog"
        aria-modal="true"
        @click.stop
      >
        ...
        <form class="flex min-h-0 flex-1 flex-col" @submit.prevent="emit('submit')">
          ...
          <footer class="flex items-center justify-end gap-2 border-t border-brand-border px-6 py-4">
            <button
              type="button"
