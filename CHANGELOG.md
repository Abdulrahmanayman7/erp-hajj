...
- **Sprint 013 — Documents & Archiving specification** (documentation only — no application code): central tenant-owned Documents with `DOC-######` sequences; private storage via `TenantStorage::DOCUMENTS` (`tenants/{id}/documents/{uuid}.ext`); original filename as metadata only; MIME/extension allow-list + 20 MiB; SHA-256 checksum; optional single polymorphic link (Contract/Meeting/Decision/Task/Employee/Organization Unit); lifecycle `active`/`archived` + hard delete; categories catalog; permissions `documents.view|upload|download|update|archive|delete|manage_categories`; stream-only authorized downloads; ADR-0010; full module docs under `docs/09-modules/09-documents/`. No versioning, multi-link pivot, confidentiality levels, malware scanner, OCR, or public URLs.
