...
- **Sprint 015 — Assets & Custodies specification** (documentation only — no application code): assets (`AST-######`) + categories; append-only custodies (`CUS-######`) with single active custody per asset; statuses `available|in_use|maintenance|damaged|retired|lost`; assign/return concurrency via asset `FOR UPDATE`; optional home warehouse + org unit metadata; no inventory auto-post / `inventory_item_id`; Documents morph `asset`/`custody`; permissions `assets.view|create|update|delete|assign|return|retire`; holder self-view (ADR-0012); full module docs under `docs/09-modules/11-assets-and-custodies/`. No Procurement, maintenance work-orders, depreciation, or custody void/correction.
