...
- Authentication module **specification** for Sprint 005 (documentation only — no application code): Sanctum SPA cookie session login/logout/`/auth/me`, email+password, remember-me, rate limiting (5/min), user account status `active`/`disabled`, tenant lifecycle login gates, password reset (forgot/reset), password policy, auth error-code catalog, audit events, Vue auth module architecture, login/forgot/reset UI, route guards, CSRF SPA flow, and Pest/Vitest matrices. Status remains **specification complete — implementation pending**.
...
> Note: the Tenant Foundation core (multi-tenancy data layer, contexts, scoping, and isolation helpers) is implemented. Authentication is **specified** for Sprint 005 but **not implemented**. No RBAC, business modules, or email verification/MFA yet; tenant-settings and platform tenant-management endpoints await RBAC; correlation ID middleware ships with auth auditing / Audit core.
