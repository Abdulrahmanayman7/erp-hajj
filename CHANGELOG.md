...
- **Sprint 009 — Contracts specification** (documentation only): tenant-owned `contract_categories` (ADR-0006, active/inactive) + `contracts` + authoritative append-only `contract_status_transitions` (correlation ID); `CTR-######` sequence numbering; counterparty free-text + optional employee/org links; Workflow 2 lifecycle actions; **manual sign attestation** (not e-sign); **one-child transactional renew** (`CONTRACT_ALREADY_RENEWED`); expiry job + expiring-soon filter; draft-only hard delete; informational value/SAR; attachments deferred; notification hooks only; API/UI/Pest/Vitest matrices. **No application code.**
...
> Note: Tenant Foundation (004), Authentication (005), and Users & Authorization / RBAC (006) are implemented. No other business modules yet.
