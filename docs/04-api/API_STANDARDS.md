...
- **Pagination** on all list endpoints (defaults/max page size: TBD); **search, filters, and sorting** supported via query parameters. A page-size cap is a **payload limit**, not a completeness guarantee: clients must not treat the first N rows as the full catalog. Relation pickers reuse these searchable, paginated list endpoints (no dedicated `/lookup/*` surface in the MVP).
