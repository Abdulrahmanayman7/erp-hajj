...
| Item | Notes |
|---|---|
| System settings API/UI (`tenant_settings`) | Permissions + `tenant_settings` table exist; dedicated settings endpoints/UI still pending (ROADMAP). Tenant timezone/locale currently driven primarily from `tenants` row / defaults. |
