...
- Warehouse / inventory lifecycle (**specified** Sprint 014 — not yet implemented): `WAREHOUSE_*`, `INVENTORY_ITEM_*`, `INVENTORY_CATEGORY_*`, `STOCK_RECEIVED`, `STOCK_ISSUED`, `STOCK_RETURNED`, `STOCK_TRANSFERRED`, `STOCK_ADJUSTED` — see [10-warehouses-and-inventory/API.md](../09-modules/10-warehouses-and-inventory/API.md). Authoritative quantity history remains `inventory_movements` (correlation ID). Payloads: before/after balances, qty, reason/reference — never storage paths.
