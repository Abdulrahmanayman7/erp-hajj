...
> **Status:** Approved (catalog may grow with modules). Sprint 006–011 permissions are seeded as implemented.
>
...
| Warehouses | `warehouses.view` `warehouses.create` `warehouses.update` `warehouses.delete` — **specified** Sprint 014 ([10-warehouses-and-inventory/PERMISSIONS.md](../09-modules/10-warehouses-and-inventory/PERMISSIONS.md); ADR-0011). Soft-delete/deactivate semantics in module docs. |
| Inventory | `inventory.view` `inventory.manage_items` `inventory.add` `inventory.issue` `inventory.return` `inventory.transfer` `inventory.adjust` — **specified** Sprint 014. `manage_items` covers item master + categories; stock mutations are separate verbs. Adjustments are sensitive. |
