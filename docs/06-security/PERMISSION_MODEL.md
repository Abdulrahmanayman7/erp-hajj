...
| Tasks | `tasks.view` `tasks.create` `tasks.update` `tasks.assign` `tasks.change_status` `tasks.complete` `tasks.delete` — **named** in catalog; **seed at Sprint 012 implementation** ([08-tasks/PERMISSIONS.md](../09-modules/08-tasks/PERMISSIONS.md)). Start/cancel/progress map to `change_status`; limited assignee self-service documented in ADR-0009. |
