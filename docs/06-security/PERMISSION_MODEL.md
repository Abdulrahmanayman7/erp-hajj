...
| Documents | `documents.view` `documents.upload` `documents.download` `documents.update` `documents.archive` `documents.delete` `documents.manage_categories` — **named** in Sprint 013 spec; **seed at implementation** ([09-documents/PERMISSIONS.md](../09-modules/09-documents/PERMISSIONS.md); ADR-0010). Archive ≠ hard delete. Do not seed `contracts.attach_documents`. |
