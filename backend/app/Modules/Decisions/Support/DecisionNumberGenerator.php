...
/**
 * Concurrency-safe per-tenant decision number allocation (DEC-######).
 */
final class DecisionNumberGenerator
{
    ...
    public function next(): string
    {
        ...
        $prefix = (string) config('decisions.number_prefix', 'DEC-');
        $pad = max(1, (int) config('decisions.number_pad', 6));

        $allocate = function () use ($tenant, $prefix, $pad): string {
            $row = DB::table('decision_number_sequences')
