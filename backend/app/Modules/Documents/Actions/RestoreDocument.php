...
final class RestoreDocument
{
    ...
    public function execute(User $actor, Document $document, ?string $comment, Request $request): Document
    {
        ...
        return DB::transaction(function () use ($actor, $document, $comment, $request, $tenant): Document {
            ...
            $locked->status = DocumentStatus::Active;
            $locked->archived_at = null;
            $locked->archived_by = null;
            $locked->save();

            $this->security->record(AuthorizationSecurityEvent::DOCUMENT_RESTORED, [
                'tenant_id' => $tenant->id,
                'actor_id' => $actor->id,
